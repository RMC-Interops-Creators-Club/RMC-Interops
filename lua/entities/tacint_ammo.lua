...
function ENT:ApplyAmmo(ply)
    ...
    local supplyamount = clipsize * 3
    local max = clipsize * GetConVar("tacint_maxclipsgiven"):GetInt()

    local t2

    if wpn.ArcticTacInt then
        ammotype = wpn.SupplyAmmoType or wpn:GetValue("Ammo")
        clipsize = wpn.ClipSize
        supplyamount = wpn.SupplyAmmoAmount or clipsize * 3
        max = clipsize * GetConVar("tacint_maxclipsgiven"):GetInt()

        if max <= 0 then
            max = GetConVar("tacint_maxclipsgiven"):GetInt()
        end
        ...
        if nade.Ammo then
            if !nade.Secret then
                t2 = ClampedGiveAmmo(ply, nade.Ammo, 1, GetConVar("tacint_maxnadesgiven"):GetInt())
            end
        end
    end
