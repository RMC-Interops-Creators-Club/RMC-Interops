...
function ENT:GiveAtt(ply)
    ...
    if GetConVar("tacint_free_atts"):GetBool() then
        ply:PrintMessage(HUD_PRINTTALK, "All attachments are free! This is not necessary!")
        return
    end

    if GetConVar("tacint_lock_atts"):GetBool() and TacInt:PlayerGetAtts(ply, self.AttToGive) > 0 then
        ply:PrintMessage(HUD_PRINTTALK, "You already have this attachment!")
        return
    end

    TacInt:PlayerGiveAtt(ply, self.AttToGive, 1)
    TacInt:PlayerSendAttInv(ply)

    self:EmitSound("tacint/weapons/flashlight_on.wav")
    self:Remove()
end
    ...
    function ENT:Draw()
        ...
        if (EyePos() - self:WorldSpaceCenter()):LengthSqr() <= 262144 then -- 512^2
            ...
            cam.Start3D2D(pos, ang, 0.1)
                surface.SetFont("TacInt_LondonBetween_32_Unscaled")
